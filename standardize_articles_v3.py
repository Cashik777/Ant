...
def process_file(file_path):
    ...
    # Skip if already layouted
    if "article-layout" in content:
        print(f"Already standardized: {file_path}")
        return

    # 1. Update CSS if missing (it might be there from previous run)
    if ".recommended-stories-widget" not in content:
        content = content.replace("    </style>", new_styles + "    </style>")

    # 2. Extract TOC items and assign IDs to H2 tags
    toc_items = []
    h2_pattern = re.compile(r'<h2(.*?)data-i18n="(.*?)"(.*?)>(.*?)</h2>', re.IGNORECASE)
    
    # We'll use a wrapper to collect toc items
    id_map = {}
    
    def collect_h2(match):
        attrs1, i18n_key, attrs2, existing_content = match.groups()
        section_id = f"section-{len(toc_items) + 1}"
        toc_items.append((section_id, i18n_key, existing_content))
        return match.group(0) # Don't replace yet

    h2_pattern.sub(collect_h2, content)
    
    # Actually perform H2 replacement with IDs (if not already there)
    # Note: If IDs were already added in previous run, we should probably check for them.
    # But for simplicity, we'll re-do it or check if id="section-X" is present.
    
    section_count = [1]
    def replace_h2_with_id(match):
        attrs1, i18n_key, attrs2, existing_content = match.groups()
        if 'id="section-' in attrs1 or 'id="section-' in attrs2:
            # Preserve existing ID
            return match.group(0)
        
        section_id = f"section-{section_count[0]}"
        new_h2 = f'<h2 id="{section_id}"{attrs1}data-i18n="{i18n_key}"{attrs2}>{existing_content}</h2>'
        section_count[0] += 1
        return new_h2

    new_content = h2_pattern.sub(replace_h2_with_id, content)

    # 3. Inject Layout
    # Find the article-body tag
    article_start_tag = '<article class="article-body">'
    article_end_tag = '</article>'
    
    start_idx = new_content.find(article_start_tag)
    end_idx = new_content.find(article_end_tag, start_idx)
    
    if start_idx != -1 and end_idx != -1:
        # Full article content (with its own tags)
        original_article_html = new_content[start_idx : end_idx + len(article_end_tag)]
        
        # Prepare TOC HTML
        toc_li_items = []
        for sid, key, text in toc_items:
            toc_li_items.append(f'                        <li><a href="#{sid}" data-i18n="{key}">{text}</a></li>')
        
        toc_html = left_sidebar_template.replace("{toc_items}", "\n".join(toc_li_items))
        
        # Build the final layout string (WITHOUT F-STRINGS to avoid curly brace issues)
        layout_injected = "        <div class=\"article-layout\">\n"
        layout_injected += toc_html + "\n"
        layout_injected += "            <div class=\"article-main\">\n"
        layout_injected += "                " + original_article_html + "\n"
        layout_injected += "            </div>\n"
        layout_injected += right_sidebar_content + "\n"
        layout_injected += "        </div>"
        
        # Replace explicitly
        new_content = new_content[:start_idx] + layout_injected + new_content[end_idx + len(article_end_tag):]
        
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Successfully standardized: {file_path}")
    else:
        print(f"Error finding article in {file_path}")
